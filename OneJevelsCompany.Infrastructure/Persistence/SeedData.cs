...
namespace OneJevelsCompany.Infrastructure.Persistence
{
    public static class SeedData
    {
        public static async Task ApplyAsync(AppDbContext db)
        {
            ...
            var catChain = await db.ComponentCategories
                .FirstAsync(c => c.Name == "Chain");

            var catClasp = await db.ComponentCategories
                .FirstAsync(c => c.Name == "Clasp");

            var catPendant = await db.ComponentCategories
                .FirstAsync(c => c.Name == "Pendant");

            var catBead = await db.ComponentCategories
                .FirstAsync(c => c.Name == "Bead");
            ...
            var compsByName = await db.Components
     ...
     .FirstOrDefaultAsync(j => j.Name == "Classic Gold Pearl Necklace");
            ...
            {
                void Link(Jewel jewel, string componentName)
                {
                    var component = compsByName[componentName];
